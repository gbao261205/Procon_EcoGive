...
public class FixDbServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            // 2. Cập nhật ENUM status cho bảng transactions
            ...
            String sqlUpdateEnum = "ALTER TABLE transactions MODIFY COLUMN status " +
                    ...
                    "'PENDING_TRADE', 'WAITING_ADMIN_APPROVAL', 'TRADE_ACCEPTED', 'TRADE_APPROVED', 'TRADE_REJECTED', " +
                    "'CONFIRMED_BY_A', 'CONFIRMED_BY_B') " +
                    "DEFAULT 'PENDING'";
            try {
                stmt.executeUpdate(sqlUpdateEnum);
                resp.getWriter().write("<p style='color:green'>✅ Updated ENUM status.</p>");
            } catch (Exception e) {
                resp.getWriter().write("<p style='color:red'>❌ Failed to update ENUM: " + e.getMessage() + "</p>");
            }

            // 3. Thêm cột transaction_type & offer_item_id
            ...
            if (rsOffer.next()) {
                ...
                try {
                    ...
                    stmt.executeUpdate("ALTER TABLE transactions ADD CONSTRAINT fk_transactions_offer_item FOREIGN KEY (offer_item_id) REFERENCES items(item_id) ON DELETE SET NULL");
            ...
            // 4. Cập nhật ENUM status cho bảng items (MỚI)
            resp.getWriter().write("<h3>4. Updating 'items' status ENUM...</h3>");
            String sqlUpdateItemEnum = "ALTER TABLE items MODIFY COLUMN status " +
                    "ENUM('AVAILABLE', 'PENDING', 'CONFIRMED', 'COMPLETED', 'CANCELLED', 'TRADE_PENDING') " +
                    "DEFAULT 'AVAILABLE'";
            try {
                stmt.executeUpdate(sqlUpdateItemEnum);
                resp.getWriter().write("<p style='color:green'>✅ Updated ITEMS ENUM status (Added TRADE_PENDING).</p>");
            } catch (Exception e) {
                resp.getWriter().write("<p style='color:red'>❌ Failed to update ITEMS ENUM: " + e.getMessage() + "</p>");
            }
