...
<head>
    <meta charset="UTF-8">
    <title><fmt:message key="admin.stations.title" /> - EcoGive Admin</title>
...
<main class="md:ml-64 min-h-screen transition-all duration-300 flex flex-col">
    <header class="bg-white border-b border-slate-200 sticky top-0 z-10 px-8 py-4 flex justify-between items-center shadow-sm">
        <div>
            <h1 class="text-2xl font-bold text-slate-800 tracking-tight"><fmt:message key="admin.stations.title" /></h1>
            <p class="text-sm text-slate-500 mt-1"><fmt:message key="admin.stations.subtitle" /></p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tr>
                        <th class="px-6 py-4 border-b border-slate-100"><fmt:message key="admin.stations.name" /></th>
                        <th class="px-6 py-4 border-b border-slate-100"><fmt:message key="admin.stations.type" /></th>
                        <th class="px-6 py-4 border-b border-slate-100"><fmt:message key="admin.stations.address" /></th>
                        <th class="px-6 py-4 border-b border-slate-100"><fmt:message key="admin.stations.owner" /></th>
                        ...
                        <tr class="group hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold bg-slate-100 text-slate-700 border border-slate-200">
                                    <span class="text-base">${st.typeIcon}</span>
                                    <!-- SỬA ĐỔI: Dùng fmt:message để dịch tên loại hình -->
                                    <c:set var="typeKey" value="type.${st.typeCode}" />
                                    <fmt:message key="${typeKey}" var="translatedTypeName" />
                                    <c:out value="${!fn:contains(translatedTypeName, '???') ? translatedTypeName : st.typeName}" />
                                </span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <span class="text-slate-400 italic text-xs"><fmt:message key="home.public_point" /></span>
                                    </c:otherwise>
                                </c:choose>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    <form action="${pageContext.request.contextPath}/admin" method="POST" onsubmit="return confirm('${confirmDelete}');" class="inline">
                                        <input type="hidden" name="action" value="delete-station">
                                        <input type="hidden" name="id" value="${st.pointId}">
                                        <button class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="<fmt:message key='admin.common.delete' />">
        ...
        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-slate-100">
                <div class="bg-white px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                    <h3 id="modalTitle" class="text-lg font-bold text-slate-800"><fmt:message key="admin.stations.add_modal" /></h3>
                ...
                <form action="${pageContext.request.contextPath}/admin" method="POST" class="p-6 space-y-5">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5"><fmt:message key="admin.stations.name" /> <span class="text-red-500">*</span></label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5"><fmt:message key="admin.stations.type" /></label>
                            <div class="relative">
                                <select id="type" name="type" class="w-full appearance-none rounded-xl border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all cursor-pointer">
                                    <c:forEach var="t" items="${types}">
                                        <!-- SỬA ĐỔI: Dịch tên loại hình trong dropdown -->
                                        <c:set var="typeKey" value="type.${t.typeCode}" />
                                        <fmt:message key="${typeKey}" var="translatedTypeName" />
                                        <option value="${t.typeCode}">${t.icon} <c:out value="${!fn:contains(translatedTypeName, '???') ? translatedTypeName : t.displayName}" /></option>
                                    </c:forEach>
                                </select>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5"><fmt:message key="admin.stations.address" /> <span class="text-red-500">*</span></label>
                            <div class="relative">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5"><fmt:message key="admin.stations.coords" /> <span class="text-red-500">*</span></label>
...
<script>
    ...
    function openModal(id, name, type, address, lat, lng) {
        ...
        setTimeout(() => {
            initMap();
            if (id) {
                document.getElementById('modalTitle').innerText = "<fmt:message key='admin.stations.edit_modal' />";
                ...
                document.getElementById('modalTitle').innerText = "<fmt:message key='admin.stations.add_modal' />";
