...
<head>
    <meta charset="UTF-8">
    <title><fmt:message key="admin.cat.title" /> - EcoGive Admin</title>
...
<main class="md:ml-64 min-h-screen transition-all duration-300 flex flex-col">
    <header class="bg-white border-b border-slate-200 sticky top-0 z-10 px-8 py-4 flex justify-between items-center shadow-sm">
        <div>
            <h1 class="text-2xl font-bold text-slate-800 tracking-tight"><fmt:message key="admin.cat.title" /></h1>
            <p class="text-sm text-slate-500 mt-1"><fmt:message key="admin.cat.subtitle" /></p>
        </div>
    </header>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sticky top-24">
                <h3 id="form-title" class="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                    ...
                    <fmt:message key="admin.cat.add_modal" />
                </h3>
                ...
                <form id="category-form" action="${pageContext.request.contextPath}/admin" method="POST" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5"><fmt:message key="admin.cat.name" /> <span class="text-red-500">*</span></label>
                        <fmt:message key="admin.cat.placeholder_name" var="catPlaceholder" />
                        ...
                        <p class="text-[10px] text-slate-400 mt-1">Nhập mã (VD: FURNITURE) để hỗ trợ đa ngôn ngữ.</p>
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5"><fmt:message key="admin.cat.points" /> <span class="text-red-500">*</span></label>
                        <div class="relative">
                        ...
                        <p class="text-[10px] text-slate-400 mt-1"><fmt:message key="admin.cat.help_text" /></p>
                    </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <h2 class="font-bold text-slate-700"><fmt:message key="admin.cat.list_title" /></h2>
                    <fmt:message key="admin.cat.count" var="catCount">
                        <fmt:param value="${categories.size()}" />
                    </fmt:message>
                    <span class="text-xs font-bold text-slate-500 bg-white border border-slate-200 px-2.5 py-1 rounded-lg shadow-sm">${catCount}</span>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tr>
                            ...
                            <th class="px-6 py-4 border-b border-slate-100"><fmt:message key="admin.cat.name" /></th>
                            <th class="px-6 py-4 border-b border-slate-100"><fmt:message key="admin.cat.points" /></th>
                        ...
                        <c:forEach var="cat" items="${categories}">
                            <tr class="group hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-6 py-4 font-semibold text-slate-800">
                                    ...
                                    <fmt:message key="${catKey}" var="translatedName" />
                                    <!-- Fallback nếu không tìm thấy key (hiển thị tên gốc) -->
                                    <c:out value="${!fn:contains(translatedName, '???') ? translatedName : cat.name}" />
                                </td>
                                <td class="px-6 py-4">
...
<script>
    function editCategory(id, name, points) {
        ...
        // UI Updates
        document.getElementById('form-title').innerHTML = '<span class="w-8 h-8 rounded-lg bg-blue-100 text-blue-600 flex items-center justify-center text-sm">✏️</span> <fmt:message key="admin.cat.edit_modal" />';
    ...
    function cancelEdit() {
        ...
        // UI Reset
        document.getElementById('form-title').innerHTML = '<span class="w-8 h-8 rounded-lg bg-emerald-100 text-emerald-600 flex items-center justify-center text-sm">✨</span> <fmt:message key="admin.cat.add_modal" />';
        const btn = document.getElementById('submit-button');
